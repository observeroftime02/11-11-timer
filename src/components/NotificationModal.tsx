...
import { X, Bell, Volume2, Check, Sparkles, Smartphone, Settings } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-950/80 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg rounded-3xl bg-neutral-900 border border-neutral-800 p-6 md:p-8 shadow-2xl space-y-5 overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        {/* Android Home Screen Widgets Button / Banner */}
        {onOpenWidgets && (
          <div className="rounded-2xl bg-neutral-950 p-4 border border-neutral-800 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-400 mt-0.5">
                  Live countdowns for 11:11 & 4:20 directly on your home screen.
                </div>
              </div>
            </div>

            <button
              id="btn-settings-open-widgets"
              onClick={() => {
                onClose();
                onOpenWidgets();
              }}
              className="px-3.5 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-750 text-neutral-200 hover:text-white text-xs font-semibold border border-neutral-700 transition-all cursor-pointer shrink-0"
            >
              Preview
            </button>
          </div>
